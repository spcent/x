...
type ContextCache interface {
	...
	GetContext(ctx context.Context, key string) any
	SetContext(ctx context.Context, key string, val any, timeout time.Duration) error
	IsExistContext(ctx context.Context, key string) bool
	DeleteContext(ctx context.Context, key string) error
}

// GetContext get value from cache
func GetContext(ctx context.Context, cache Cache, key string) any {
...
func SetContext(ctx context.Context, cache Cache, key string, val any, timeout time.Duration) error {
	if cache, ok := cache.(ContextCache); ok {
