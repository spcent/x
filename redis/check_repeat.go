...
func (c *checkRepeat) Set(key string, value any, expiration time.Duration) error {
	key = getKey(key)
...
func (c *checkRepeat) SetNX(key string, value any, expiration time.Duration) (bool, error) {
	key = getKey(key)
